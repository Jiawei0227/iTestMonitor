...
PROMETHEUS_SCRIPT = os.path.join(LIB_DIR, 'prometheus.sh')
...
@fabric.task
@fabric.parallel(pool_size=FABRIC_MAX_POOL_SIZE)
def install_node_exporter():
  return fabric.run(dedent("""
    ...
    cd /opt/prometheus
    wget --quiet {S3_BUILD_URL}/{PROMETHEUS_GBN}/node_exporter-{NODE_EXPORTER_VERSION}.linux-amd64.tar.gz
...
@fabric.task
def install_prometheus_server(local_host_target, remote_hosts_target, impalad_hosts_target):
  prometheus_yml=get_prometheus_yml(local_host_target, remote_hosts_target, impalad_hosts_target)
  return fabric.run(dedent("""
    useradd --no-create-home --shell /bin/false prometheus
    # prometheus_server
    ...
    cd /opt/prometheus
    wget --quiet {S3_BUILD_URL}/{PROMETHEUS_GBN}/prometheus-{PROMETHEUS_VERSION}.linux-amd64.tar.gz
    tar -xzf prometheus-{PROMETHEUS_VERSION}.linux-amd64.tar.gz
    rm prometheus-{PROMETHEUS_VERSION}.linux-amd64.tar.gz
    cat > /etc/systemd/system/prometheus.service<<EOF{PROMETHEUS_SERVICE}EOF
    cat > /opt/prometheus/prometheus.yml<<EOF{PROMETHEUS_YML}EOF
    systemctl daemon-reload
  """).format(S3_BUILD_URL=S3_BUILD_URL,
